...
- `.env` 文件已配置（OSS_KEY_ID / OSS_KEY_SECRET）
...
```bash
grep -rn "LTAI\|AccessKey\|password\s*=\s*['\"]" --include="*.py" --include="*.js" --include="*.env*" .
```

- 如发现密钥硬编码：先执行 `git-secret-scan` Skill 修复
- 确认 `.env` 和 `.env.local` 不在 git 跟踪中：`git ls-files | grep -i env`
