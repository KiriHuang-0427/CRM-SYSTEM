...
function migrateAIReviewFields() {
  ...
  if (!cols.includes('review_status')) {
    ...
    console.log('[DB] ai_memories: added review_status column');
  }
  if (!cols.includes('review_note')) {
    db.exec("ALTER TABLE ai_memories ADD COLUMN review_note TEXT");
    console.log('[DB] ai_memories: added review_note column');
  }
  if (!cols.includes('reviewed_at')) {
    db.exec("ALTER TABLE ai_memories ADD COLUMN reviewed_at TEXT");
    console.log('[DB] ai_memories: added reviewed_at column');
  }

  // Batch update: records with customer_id get 'linked' status
